...
TEST = True
TEST_SOLUTION = 29 # add solution for test input here
...
def puzzle(filecontent):
    ...
    for x, y in starting_points:
        length = find(grid, end_x, end_y, x, y)
        print(starting_points.index((x, y)))
        if(length < shortest):
            shortest = length
...
def find(grid, end_x, end_y, start_x, start_y):
    result = 0
